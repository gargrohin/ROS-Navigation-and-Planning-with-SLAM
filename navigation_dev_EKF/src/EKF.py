...
class jetbotEKF():
    ...
    ## Constructor to initialize the state, measurement and noise matrices
    def __init__(self):
        ss = 3
        ms = ss-3
        self.ss = 3 ## state vector size
        self.ms = ss-3 ## measurement vector size
        ...
        self.Q = np.array([[0. for i in range(ss)] for j in range(ss)]) ## Noise Matrix for System (motion)
        for i in range(2):
            self.Q[i,i] = 0.15
        self.Q[2,2] = np.pi/10
        for i in range(3,ss):
            self.Q[i,i] = 0.0
        self.R = np.array([[0. for i in range(ms)] for j in range(ms)]) ## Noise matrix for measurements
        for i in range(ms):
            self.R[i,i] = 0.1
    ...
    ## method that checks how many unique landmarks we have
    ...
    ## method to add a new landmark and subsequently update the state, measurement and noise matrices
    ...
    ## mthod to predict apriori state
